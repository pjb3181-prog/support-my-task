...
/** 최초 실행 seed. 사용자가 삭제한 built-in 업무유형은 다시 만들지 않는다. */
object SeedData {

    suspend fun seed(db: AppDatabase) {
        val templateDao = db.templateDao()
        val settingDao = db.settingDao()
        val firstTemplateSeed = templateDao.countTemplates() == 0

        if (firstTemplateSeed) {
        ...
        if (firstTemplateSeed && templateDao.countScheduleTypeRules() == 0) {
