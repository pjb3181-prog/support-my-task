 ...
 * 3) isMine: attendeeCode 내부에 현재 사용자의 식별문자가 있는지만 확인
 ...
 * 6) isTarget = isMine
 *
 * mineMarker는 사용자별 설정값이며, 기본값은 기존 사용자 호환을 위해 "종"이다.
 */
class EventTitleParser {

    fun parse(
        rawTitle: String,
        rules: List<ScheduleTypeRule>,
        mineMarker: String = DEFAULT_MINE_MARKER
    ): ParsedTitle {
        ...
        val marker = mineMarker.trim()
        val isMine = marker.isNotEmpty() && attendeeCode?.contains(marker) == true
    ...
    companion object {
        ...
        const val DEFAULT_MINE_MARKER = "종"
